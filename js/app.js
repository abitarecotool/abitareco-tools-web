...
window.addEventListener('error', (e) => {
  if (e.filename?.includes('qrcode') || e.message?.includes('QRCode')) {
    console.error('Errore QRCode:', e.message);
  }
}, true);
...
async function exportWithWebCodecsMP4(items, {T,F,fps,W,H,bitrate}){
  ...
  const cfg = await supportsH264WebCodecs(); const mp4 = MP4Box.createFile();
  const chunks = [];
...
async function makeQr(){
  ...
  // Verifica QRCode disponibile
  if (!window.QRCode) {
    alert('Libreria QRCode non caricata. Ricarica la pagina e riprova.');
    console.error('QRCode lib not available');
    return;
  }
  ...
  // Crea ZIP con PNG e SVG
  const zip = new JSZip();
  
  // PNG
  const pngBlob = await new Promise(res => {
    QrCanvas.toBlob(b => { res(b || new Blob()); }, 'image/png');
  });
  zip.file('qr.png', pngBlob);

  // SVG
  const svgStr = await QRCode.toString(url, { type:'svg', margin:1, width:256 });
  zip.file('qr.svg', new Blob([svgStr], {type:'image/svg+xml'}));

  // Scarica ZIP
  const zipBlob = await zip.generateAsync({type:'blob'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(zipBlob);
  a.download = `QR-${slugify((QrCampaign?.value || 'qrcode'))}-${Date.now()}.zip`;
  a.click();
  URL.revokeObjectURL(a.href);

  // Mantieni i link per download manuale
