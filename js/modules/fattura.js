...
(function(){
 ...
 function addRowFromSelected(){
  ...
  acc.type = 'text';
  acc.placeholder = 'Acconto (es. Maggio)';
 ...
 function getState(){
  ...
  const rows = Array.from(document.querySelectorAll('#FatRows .fattura-row')).map(r => {
   ...
   return { product: prod, acc, qty, unit, disc, base, total };
  });
 ...
 function drawRow(page, font, x, y, cols, row, anyDiscount, rgb, alt){
  ...
  const values = [];
  values.push(trunc(row.product, 36));
  values.push(trunc(row.acc || '', 18));
 ...
 async function exportFatturaPdf(){
  ...
  const cols = st.anyDiscount ? [
    ...
    { label:'Prodotto', w: 240 },
    { label:'Acconto', w: 90 },
    { label:'Q.tà', w: 45, align:'right' },
    { label:'Costo unit.', w: 90, align:'right' },
    { label:'Totale', w: 100, align:'right' },
  ];

  const contentW = cols.reduce((a,c)=>a+c.w,0);
  const marginX = (A4[0] - contentW) / 2;
  const marginTop = 40;
  ...
  const rightBoxW = 195;
  const rightBoxX = marginX + contentW - rightBoxW;
  ...
  ensureSpace(160);
  drawTableHeader(page, fontBold, marginX, y, cols, rgb);
  y -= 18;
  ...
  st.rows.forEach((r, idx) => {
   ...
   y = drawRow(page, font, marginX, y, cols, r, st.anyDiscount, rgb, idx % 2 === 1);
  });

  ensureSpace(130);
  y -= 10;
  page.drawLine({ start: { x: marginX, y }, end: { x: marginX + contentW, y }, thickness: 1, color: rgb(0.86,0.86,0.86) });
  y -= 18;
  page.drawText('Totale fornitura', { x: marginX + contentW - 170, y, size: 11, font: fontBold, color: rgb(0.07,0.09,0.12) });
  page.drawText(euro(st.total), { x: marginX + contentW - 2, y, size: 11, font: fontBold, color: rgb(0.77,0.09,0.17) });
