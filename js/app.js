/* =========================================================
   ...
   app.js — Immagini + DigitalTool + PDF→JPG + Rename + Video
   ========================================================= */
"use strict";
...
async function exportImages(){
  ...
  if (!slugIta || !slugEng){
    alert("Compila i campi ITA e ENG."); return;
  }
  const images = picked.filter(p => /\.(jpe?g|png|tif?f)$/i.test(p.file.name));
  if (!images.length){
    alert("Carica una cartella con immagini."); return;
  }
...
/* ---- Export: MediaRecorder (time‑based, stop a T esatto) ---- */
async function exportWithMediaRecorder(items, {T,F,fps,W,H,mime,bitrate}){
  ...
  // Loop di disegno legato al TEMPO reale (non al conteggio frame)
...
async function exportVideoSlideshow(){
  ...
  // Download diretto (anteprima rimossa su richiesta)
