...
/* ===================================================
 🎞️ VIDEO SLIDESHOW — SOLO CLIENT-SIDE (NEW)
  - Formati: Orizzontale (1920×1080), Verticale (1080×1920), Quadrato (1080×1080)
  - Durate: 15/30/45s, Dissolvenza fissa 1s, FPS 30
  - MP4 preferito (WebCodecs+MP4Box o MediaRecorder su Safari), fallback WebM
=================================================== */
...
const VidFade     = { value: '1' }; // default fisso 1s (non in UI)
const VidFps      = { value: '30' }; // default fisso 30fps (non in UI)
...
if (DropAreaVideo) {
  ...
  BtnClearVideo?.addEventListener('click', (e)=>{
    ...
    VidPreview.removeAttribute('src'); VidPreview.load();
  });
}

// Feature detection (MP4 / WebCodecs)
...
function renderAt(timeline, items, W, H, tSec) {
  const { still, fade, offsets } = timeline;
...
async function filesToBitmapsVideo(recs) {
  const arr = [];
  for (const r of recs) {
    const bmp = await loadImageBitmap(r.file);
    arr.push({ name: r.file.name, bmp });
  }
  return arr;
}

// Export con WebCodecs (H.264) + MP4Box
async function exportWithWebCodecsMP4(items, {T, F, fps, W, H}) {
  ...
  const timeline = buildTimelineVideo(items.length, T, F, fps);

  const cfg = await supportsH264WebCodecs();
  if (!cfg) throw new Error('H.264 WebCodecs non disponibile');
  const encConfig = { ...cfg, width:W, height:H, framerate:fps, bitrate: 5_000_000, avc:{ format:'annexb' } };
  ...
  const encoder = new VideoEncoder({
    output: (chunk, meta) => {
      const ts = chunk.timestamp; // microsecondi
  ...
  for (let f=0; f<totalFrames; f++) {
    renderAt(timeline, items, W, H, f / fps);
    const frame = new VideoFrame(VidCanvas, { timestamp: f * frameDurUs });
    encoder.encode(frame, { keyFrame: (f===0) || (f % (fps*2) === 0) });
    frame.close();

    if ((f % fps) === 0) {
      ActionProgress.value = Math.round((f/totalFrames)*100);
...
// Export con MediaRecorder (MP4 su Safari, WebM fallback)
async function exportWithMediaRecorder(items, {T, F, fps, W, H}, mime) {
  showEl(ActionProgressWrap);
  ActionProgress.value = 0;
...
async function exportVideoSlideshow(){
  ...
  const F   = Math.max(0, parseFloat(VidFade.value)); // 1s fisso
  const fps = Math.min(60, Math.max(10, parseInt(VidFps.value,10) || 30));
  ...
  if (h264Cfg && window.MP4Box) {
    blob = await exportWithWebCodecsMP4(items, { T, F, fps, W, H });
    filename = `${title}.mp4`;
  } else if (mp4Mime) {
    blob = await exportWithMediaRecorder(items, { T, F, fps, W, H }, mp4Mime);
    filename = `${title}.mp4`;
  } else {
    const webmMime = (window.MediaRecorder && MediaRecorder.isTypeSupported('video/webm;codecs=vp9'))
      ? 'video/webm;codecs=vp9' : 'video/webm;codecs=vp8';
    blob = await exportWithMediaRecorder(items, { T, F, fps, W, H }, webmMime);
    filename = `${title}.webm`;
  }

  // Anteprima + Download
  const url = URL.createObjectURL(blob);
  VidPreview.src = url; VidPreview.load();
...
BtnProcedi?.addEventListener('click', async ()=>{
  try {
    ...
    if (currentMode === 'video'){ await exportVideoSlideshow(); return; } // <-- NEW
