...
BtnProcedi?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (mode === 'slidebuilder') {
      if (typeof window.exportSlideBuilderPpt !== 'function') {
        throw new Error('Slide Builder non pronto: ricarica la pagina e riprova.');
      }
      await window.exportSlideBuilderPpt(); ok = true; return;
    }
    ...
    // Reset post-export per TUTTE le modalità tranne Iubenda / Platform / SlideBuilder
    if (ok && !['iubenda','platform','slidebuilder'].includes(mode)) {
